...
object Breakfast extends ZIOAppDefault {
  ...
  def makeBreakfast(eggsFiringTime: Duration,
    ...
    for {
      before <- ZIO.succeed(LocalDateTime.now())

      water <- ZIO.sleep(waterBoilingTime).as(before.plusSeconds(waterBoilingTime.toSeconds)).fork
      eggs <- ZIO.sleep(eggsFiringTime).as(before.plusSeconds(eggsFiringTime.toSeconds)).fork
